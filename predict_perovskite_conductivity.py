...
def get_preds_ebars_domains(df_test):
    d = 'model_perovskite_conductivity'
...
def make_predictions(comp_list, temp_list, ph2o_list, po2_list):

    # Process data
    df_test = process_data(comp_list)

    # Add other features to df_test
    df_test['Temperature (degC)'] = temp_list
    df_test['PH2O (%)'] = ph2o_list
    df_test['PO2 (%)'] = po2_list

    # Get the ML predicted values
    preds, ebars, domains = get_preds_ebars_domains(df_test)

    pred_dict = {'Compositions': comp_list,
                 'Predicted log perovskite conductivity (S/cm)': preds,
                 'Ebar log perovskite conductivity (S/cm)': ebars}
